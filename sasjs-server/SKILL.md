...
Download the relevant zip from GitHub releases, **verify it before executing**: pin a specific release tag (not `latest`), check the published SHA256 checksum, or — better — build from source.
...
- Input tables (the `sasjs_tables` mechanism) arrive **either** as an inline CSV const **or** — when the adapter sends multipart — as an uploaded `<name>.csv` file in the session folder, referenced by generated module-scope consts (handle BOTH):
  ...
  - these consts are **not on `globalThis`** — read them via `this['+name+']` (avoid `eval()` where possible; treat any `eval` on attacker-influenced content as a red flag) or a `typeof` guard in module scope (server-side JS, no CSP)
